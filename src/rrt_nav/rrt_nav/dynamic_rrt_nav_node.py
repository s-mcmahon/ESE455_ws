...
class RRTPlannerNav2(Node):
    def __init__(self):
        ...
        # ------------- sensing / costmap params -------------
        # For global:  /global_costmap/costmap  with frame "map"
        # For local:   /local_costmap/costmap   with frame "odom"
        self.declare_parameter('map_topic', '/local_costmap/costmap')
        self.declare_parameter('global_frame', 'odom')
        self.declare_parameter('map_is_costmap', True)       
        self.declare_parameter('costmap_occ_thresh', 253)   
        self.declare_parameter('map_qos_transient', False)   
        ...
        # ----- pubs/subs -----
        # Subscribe to (global or local) costmap
        topic = str(self.get_parameter('map_topic').value)
        if bool(self.get_parameter('map_qos_transient').value):
        ...
        self.get_logger().info(
            f"RRTPlannerNav2 ready. Using grid from {topic} "
            f"(costmap={bool(self.get_parameter('map_is_costmap').value)}), "
            f"frame={self.get_parameter('global_frame').value}."
        )
    ...
    def on_map(self, msg: OccupancyGrid):
        ...
        raw = np.array(msg.data, dtype=np.int16).reshape(self.h, self.w)
        grid = np.zeros_like(raw, dtype=np.int8)

        if bool(self.get_parameter('map_is_costmap').value):
            # Nav2 costmap semantics:
            #   -1 unknown, 0..252 cost, 253 inscribed, 254 lethal
            occ_thresh = int(self.get_parameter('costmap_occ_thresh').value)
            grid[raw == -1] = -1
            grid[raw >= occ_thresh] = 1
        ...
        # Inflate obstacles for robot radius + safety margin
    ...
    def is_free_xy(self, x, y):
        ...
        if v == 1:
            return False
        if v == -1 and not bool(self.get_parameter('unknown_is_free').value):
            return False
        return True
    ...
    def tick(self):
        ...
        # Plan global (or local) path on the currently sensed grid
        path = self.rrt_once(
            start=pose,
            goal=goal,
        ...
        # Local fallback if global fails
        if len(path) < 2:
            self.get_logger().warn("RRT: no path, trying local goal...")
            sx, sy = pose
        ...
        # Pose echo for debug
        ps = PoseStamped()
        ps.header.frame_id = str(self.get_parameter('global_frame').value)
    ...
    def get_pose(self):
        try:
            frame = str(self.get_parameter('global_frame').value)
            t = self.tf.lookup_transform(frame, 'base_link', rclpy.time.Time())
...
def main():
    ...
    try:
        ...
        node.get_logger().info("Shutting down RRTPlannerNav2…")
    finally:
        if rclpy.ok():
            rclpy.shutdown()
